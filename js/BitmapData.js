...
function hexToRGB (hex) {
    return { r: ((hex & 0xff0000) >> 16), g: ((hex & 0x00ff00) >> 8),
             b: ((hex & 0x0000ff)) };
};
...
function BitmapData(width, height, transparent, fillColor, canvas) {
	...
	sourceRect = clipRect || new Rectangle(0, 0, source.width,
                                               source.height);
	...
	this.copyCanvas(this.drawingCanvas, sourceRect,
                        new Point(sourceRect.x, sourceRect.y), blendMode);
    };
	...
	for (var y=0; y<dh; y++) {
	    for (var x=0; x<dw; x++) {
		sourcePos = ((x+sx) + (y+sy) * sw) * 4;
		sourceHex = RGBToHex({ r:sourceData[sourcePos],
                                       g:sourceData[sourcePos+1],
                                       b:sourceData[sourcePos+2] });
